...
//data stores all the content which is to be shown at the main page
let data=[
    {
      "id": "1",
      "name": "Lura Senger",
...
//function which will set the attributes and content of each button
let setButton=(button,cr)=>
{
    button.setAttribute("class","btn btn-outline-info");
    button.setAttribute('aria-pressed','false');
    button.setAttribute("type","button");
    button.setAttribute("id",`${cr}`);
    button.innerHTML=cr;
}

//a function which will be used to set the columns of row of container1
let setCol=(col,data)=>
{
    col.setAttribute("class","col-md-5 col-11 mt-3 mb-3 bg-light align-self-center");
    col.style.textAlign="center";
    col.style.border="5px solid black"
    col.innerHTML=`<b>Id :</b> ${data.id}<br><b>Name :</b> ${data.name}<br><b>E-Mail :</b> ${data.email}`;
}

//iterating over all the data members and adding them to each page or row(each page or row will contain max of 10 data)
for(let i=0;i<data.length;i++)
{
    ...
    else if(currsize==10)
    {
        ...
        row1.setAttribute("class","row justify-content-center m-0");
    }
}
if(currsize>0)
{
...
setButton(prebtn,"prev");
prev.append(prebtn);
let nextbtn=document.createElement('button');
setButton(nextbtn,"next");
next.append(nextbtn);
let first=0;
let last=6;
for(let i=first;i<=last && i<btns.length;i++)
{
    buttons.append(btns[i]);
}
let update=()=>{
    container1.innerHTML="";
    container1.append(rows[cb]);

    document.getElementById(`${cb+1}`).setAttribute('class','btn btn-outline-info active');
    page=document.getElementById('page');
    page.innerHTML=`Page ${cb+1} of ${cr}`;
    prebtn.disabled=(cb-1<0)?true:false;
    nextbtn.disabled=(cb+1>=btns.length)?true:false;
    if(cb==first && cb-1>=0)
    {
        document.getElementById(`${last+1}`).remove();
        last=last-1;
        first=first-1;
        document.getElementById('buttons').prepend(btns[first]);
        
    }
    else if(cb==last && cb+1<btns.length)
    {
        document.getElementById(`${first+1}`).remove();
        last=last+1;
        first=first+1;
        document.getElementById('buttons').append(btns[last]);
    }
}
update();
...
{
    btns[i].onclick=()=>{
      document.getElementById(`${cb+1}`).setAttribute('class','btn btn-outline-info');
      cb=i;
      update();
    }
    prebtn.onclick=()=>{
